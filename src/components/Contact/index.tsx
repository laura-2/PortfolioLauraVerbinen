...
export default function Contact({ imagem, alt, titulo }: ContactProps) {
  ...
  const onSubmit: SubmitHandler<IWriteForm> = async (data) => {
    try {
      ...
      setMessageText('Mensagem enviada com sucesso!');
      setShowMessage(true);
      reset();
  ...
  return (
    <section className="flex justify-center items-center my-10 mr-10" id="Contact">
      <div className="flex w-11/12">
        <img alt={alt} src={imagem} />
      </div>
      <div className="block self-center gap-5 w-4/5">
        ...
        <form className="flex my-5 flex-col" onSubmit={handleSubmit(onSubmit)}>
          ...
          <button className="button-contact w-1/5 flex justify-center items-center gap-3">Send<img src={sendIcon} alt="Enviar"/></button>
        </form>
        {showMessage && (
